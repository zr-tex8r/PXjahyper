...
\newcommand{\PkgVersion}{1.6b}
\newcommand{\PkgDate}{2026/05/22}
...
\item |.out|ファイルの先頭に日本語文字を含むコメントを出力する。
\end{itemize}
...
\item |nocheck-driver|\Means
  既定では\Pkg{hyperref}のドライバオプションがグローバルで指定されて
  いるかを検査してそうでない場合は警告するが、その警告を抑止する。
...
\Note {\upLaTeX}においては「JIS符号値」と「Unicode符号値」の一方を
省略できる。
（省略する場合は\,|{}|\,だけ書く。）
{\pLaTeX}でもUnicode文字の出力が可能な状況であれば「JIS符号値」の方
\footnote{「Unicode符号値」も省略可能な場合もあるが、
その判断は困難であるため「Unicode符号値」の省略は推奨されない。}%
は省略できる。
（そもそもJIS~X~0208にない文字の場合は省略するしかない。）
...
\item \Pkg{jsclasses}のクラスの命令\Means
  |\maybeblue| |\HUGE|
\item \Pkg{jlreq}クラスの命令%
  \footnote{この他に、無効化とは少し異なるが、「|\|＋全角空白」
    および「|\？|\,|\！|\,等の区切り記号命令」についても
    PDF文字列中で使用可能にしている。}%
\Means
  |\jafontsize| |\tatechuyoko| |\jidori| |\akigumi| % |\jaspace|
\end{itemize}
...
\footnote{“unicodeモード”を有効にする方法は|unicode|(|=true|)\,%
  の指定以外にも存在する。
  また、\Pkg{hyperref}の7.00g版[2021-02-04]より、全てのエンジン
  （元々{\pLaTeX}は除外されていたが、2023年以降は{\pLaTeX}が
  {\TLe-(u)\pTeX}上で動作に変わったため除外でなくなった）
  について“unicodeモード”は既定で有効になっていることに注意。}%
...
\item PDF文字列中の和文文字やLICR命令の処理は
  \Pkg{hyperref}の側に任せて\Pkg{pxjahyper}では行わない。
...
\item \Pkg{hyperref}の（|pdftitle|等の）パッケージオプション中での
  ...
  \footnote{\Pkg{hyperref}の仕様として、特殊な文字を含む文書情報は
  |\hypersetup|で指定することが推奨されている。}。
\end{itemize}
